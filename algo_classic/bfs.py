...
class BFS:
    ...
    def save_visual_result(self, path, nodes_expanded, filename="bfs_discrete_path.png"):
        plt.figure(figsize=(9, 9))
        plt.imshow(self.grid, cmap='binary')
        
        if path:
            py, px = zip(*path)
            plt.plot(px, py, color='#e74c3c', linewidth=3, label='BFS Path')
            ...
            path_len = len(path)
            plt.text(self.size / 2, -1.0, f"Total Path Length: {path_len}", 
                     fontsize=15, color='#e74c3c', fontweight='bold', ha='center')
        
        plt.title(f"BFS Shortest Path (Grid {self.size}x{self.size})\nNodes Expanded: {nodes_expanded}", pad=25)
        plt.legend(loc='upper right')
        plt.savefig(filename, bbox_inches='tight')
        plt.close()

def run_comprehensive_stats(n_runs=20):
    sizes = [10, 20, 30, 40, 50, 60]
    all_results = []
    
    print(f"Đang thực hiện kiểm thử (Mỗi size chạy {n_runs} lần)...")
    
    for s in sizes:
        for _ in range(n_runs):
            solver = BFS(grid_size=s, obstacle_prob=0.15)
        ...
        print(f"    - Hoàn thành thu thập dữ liệu size {s}x{s}")
    ...
    summary.to_csv("bfs_statistical_summary.csv", index=False, encoding='utf-8-sig')
    print("\n Đã tạo file thống kê tổng hợp: bfs_statistical_summary.csv")
...
if __name__ == "__main__":
    ...
    run_comprehensive_stats(n_runs=20)